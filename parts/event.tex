...
\begin{table}[ht]
\caption[Event spotting using GMM/VSM on CNN (on pre-processed input) features (Basketball dataset) ]{ Precision \& Recall  of GMM/VSM on CNN  features (Basketball dataset)}
...
\begin{table}[ht]
\caption[Event classification using DTW on CNN features (OSUPEL basketball dataset) ]{True positive \& False positive using DTW on CNN features (basketball dataset)}
...
Figure.\ref{fig:dtw_score} describe \textit{Target score vs Non-target} score of basketball dataset and MSR dataset.  From the graph, we can understand that by using DTW on CNN features we cannot perform event spotting.
